  ...
  <div class="flex items-center gap-4">
    ...
    <div class="flex items-center gap-4 flex-1">
      <div class="w-14 h-14 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-xl">
        {data.worker.name.split(' ').map((n: string) => n[0]).join('')}
      </div>
      <div>
        <h2 class="text-2xl font-bold text-slate-900">{data.worker.name}</h2>
        <p class="text-slate-500">{data.worker.email || 'No email'}</p>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        {#if data.worker.notes}
          <div class="mt-4 pt-4 border-t border-slate-100">
            <dt class="text-sm text-slate-500">Notes</dt>
            <dd class="text-slate-700 mt-1">{data.worker.notes}</dd>
          </div>
        {/if}
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="space-y-3">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-semibold text-slate-900">{data.projects.reduce((s: number, p: any) => s + (p.allocated_hours || 0), 0)}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-sm text-slate-500">Member Since</span>
            <span class="font-semibold text-slate-900">{new Date(data.worker.created_at).toLocaleDateString()}</span>
          </div>
        </div>
      </div>
    </div>
